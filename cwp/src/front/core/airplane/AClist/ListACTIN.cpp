...
CListActIn::CListActIn(CMaster *pMaster, QWidget *pParent) :
...
{
    setListType(QString("ACT_IN"));

    fillSortModel();

    m_sortModel.SetRowFilterType(CListACSortModel::TYPE_ACT_IN);

    //but_show_all_ = new QPushButton(this);
    //but_show_all_->setCheckable(true);
    //but_show_all_->setChecked(true);
    //m_ControlsLayout->addWidget(but_show_all_);

    m_ControlsLayout->addItem(new QSpacerItem(1, 1, QSizePolicy::Expanding, QSizePolicy::Fixed));

    but_act_in_ = new QPushButton(this);
    m_ControlsLayout->addWidget(but_act_in_);

    connect(but_act_in_, SIGNAL(clicked()), this, SLOT(onButtonActIn()));
    //connect(but_show_all_, SIGNAL(clicked(bool)), &m_sortModel, SLOT(setShowAll(bool)));
...
void CListActIn::Translate()
{
    //but_show_all_->setText(tr("SHOW ALL"));
    //qDebug()<<"CListActIn";
    but_act_in_->setText(tr("ACT IN"));
    //SizeToContents();
}

int CListActIn::InsertRow(const QSharedPointer<CAirplane> &airplane)
{
    Q_UNUSED(airplane)
    return -1;
}

void CListActIn::ModifyRow(int row, const QSharedPointer<CAirplane> &airplane)
{
    Q_UNUSED(airplane)
}

bool CListActIn::IsCanDeleteRow(int row, const QSharedPointer<CAirplane> &airplane)
{
    Q_UNUSED(airplane)
    return false;
}

void CListActIn::TimerSlot()
{
}

void CListActIn::onButtonActIn()
{
    InitActIn();
...
void CListActIn::OnDblClickItemEdit(const QModelIndex &index)
{
    if (index.isValid()) {
        if (master()->getRedisDB()) {
...
void CListActIn::setInSector(QSharedPointer<CSFPL> &sfpl)
{
    QString next_sector;
    if (CMaster *warehouse = master())
    {
...
void CListActIn::InitActIn()
{

    edit_model_ = new ActTableModel(ActTableModel::act_in, 0, this,getFir2Points());
    act_in_widget_ = new ActWidget(tr("ACT IN"), edit_model_, this);
...
void CListActIn::InitActDep()
{
    edit_model_ = new ActTableModel(ActTableModel::act_dep, 0, this);
    act_in_widget_ = new ActWidget(tr("ACT DEP"), edit_model_, this);
