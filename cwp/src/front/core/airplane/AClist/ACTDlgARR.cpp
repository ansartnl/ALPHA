...
CACTDlgARR::CACTDlgARR(CMaster* pMaster, int code, quint32 prevSFPLID, QWidget *parent) :
...
//		QSharedPointer<CView> pView = pMaster->GetView();
	...
	{
        if(sAirportList.contains(itList->GetDestAerodrome()))
        {
...
void CACTDlgARR::OnChangeACID(const QString& text)
{
...
void CACTDlgARR::SelFPLinView(QString sACID)
{
    if(m_pViewFPL)
    {
...
void CACTDlgARR::OnSelFPLinView(const QString& sID)
{
    QString sACID;
