	...
	IF ac_flag = 777 THEN
		...
		ELSE
			...
			IF INSERTING THEN
				INSERT INTO ARINC_CHANGES(STATEMENT)
				VALUES
				('INSERT INTO AIRPORT(ICAO, NAME, LATITUDE, LONGITUDE, ALTITUDE) ' ||
					'VALUES(''' || :new.ICAO || ''', ''' || :new.NAME || ''', ''' || :new.LATITUDE || ''', ''' || :new.LONGITUDE || ''', ''' || :new.ALTITUDE || ''')');
			--UPDATE
			ELSE
				INSERT INTO ARINC_CHANGES(STATEMENT)
				VALUES
...
END;
/
ALTER TRIGGER AIRPORT_AC ENABLE;
