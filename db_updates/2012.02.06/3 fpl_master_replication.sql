CREATE OR REPLACE
TRIGGER FPL_MASTER_BEFORE_INSERT 
...
END;
/
ALTER TRIGGER FPL_MASTER_BEFORE_INSERT ENABLE;


CREATE OR REPLACE
TRIGGER FPL_MASTER_REPLICATOR 
      AFTER INSERT OR UPDATE OR DELETE ON FPL_MASTER FOR EACH ROW
--WHEN ( new.fpl_guid IS NOT NULL OR old.fpl_guid IS NOT NULL )
DECLARE
	curtablename CONSTANT REPLICATION_TABLE.table_name%TYPE := 'FPL_MASTER';
	lastinsertid NUMBER;
	curguid RAW(16);
...
END;
/
ALTER TRIGGER FPL_MASTER_REPLICATOR ENABLE;
