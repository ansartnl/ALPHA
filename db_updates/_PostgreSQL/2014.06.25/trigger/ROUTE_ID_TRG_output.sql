...
DROP TRIGGER IF EXISTS route_id_trg ON route CASCADE;
CREATE OR REPLACE FUNCTION trigger_fct_route_id_trg () RETURNS trigger AS $BODY$
BEGIN
  if coalesce(NEW.id::text, '') = '' then
    SELECT  nextval('route_id_seq') INTO NEW.id ;
  end if;
RETURN NEW;
END
$BODY$
...
CREATE TRIGGER route_id_trg
	BEFORE INSERT ON route FOR EACH ROW
	EXECUTE PROCEDURE trigger_fct_route_id_trg();
