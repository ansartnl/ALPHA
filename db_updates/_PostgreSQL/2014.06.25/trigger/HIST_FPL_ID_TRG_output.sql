...
DROP TRIGGER IF EXISTS hist_fpl_id_trg ON hist_fpl CASCADE;
CREATE OR REPLACE FUNCTION trigger_fct_hist_fpl_id_trg () RETURNS trigger AS $BODY$
BEGIN
  if coalesce(NEW.hist_id::text, '') = '' then
    SELECT "HIST_FPL_ID_SEQ".nextval INTO NEW.HIST_ID ;
  end if;
RETURN NEW;
END
$BODY$
...
CREATE TRIGGER hist_fpl_id_trg
	BEFORE INSERT ON hist_fpl FOR EACH ROW
