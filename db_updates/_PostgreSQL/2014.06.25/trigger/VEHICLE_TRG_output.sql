...
DROP TRIGGER IF EXISTS vehicle_trg ON vehicle CASCADE;
CREATE OR REPLACE FUNCTION trigger_fct_vehicle_trg () RETURNS trigger AS $BODY$
BEGIN
  <<COLUMN_SEQUENCES>>
  BEGIN
    IF coalesce(NEW.ID::text, '') = '' THEN
      SELECT nextval('vehicle_seq') INTO NEW.ID ;
    END IF;
...
CREATE TRIGGER vehicle_trg
	BEFORE INSERT ON vehicle FOR EACH ROW
	EXECUTE PROCEDURE trigger_fct_vehicle_trg();
