...
DROP TRIGGER IF EXISTS fpl_lt ON fpl CASCADE;
CREATE OR REPLACE FUNCTION trigger_fct_fpl_lt () RETURNS trigger AS $BODY$
BEGIN
    -- Touch the row
      update sys_tables_info set lasttouch = now() AT TIME ZONE 'UTC' where tablename ='FPL';
    RETURN NEW;
END
$BODY$
 LANGUAGE 'plpgsql';

CREATE TRIGGER fpl_lt
	AFTER INSERT OR UPDATE OR DELETE ON fpl FOR EACH ROW
	EXECUTE PROCEDURE trigger_fct_fpl_lt();
