...
DROP TRIGGER IF EXISTS route_lt ON route CASCADE;
CREATE OR REPLACE FUNCTION trigger_fct_route_lt () RETURNS trigger AS $BODY$
BEGIN
    -- Touch the row
      update sys_tables_info set lasttouch = now() AT TIME ZONE 'UTC' where tablename ='ROUTE';
    RETURN NEW;
END
$BODY$
 LANGUAGE 'plpgsql';

CREATE TRIGGER route_lt
	AFTER INSERT OR UPDATE OR DELETE ON route FOR EACH ROW
	EXECUTE PROCEDURE trigger_fct_route_lt();
