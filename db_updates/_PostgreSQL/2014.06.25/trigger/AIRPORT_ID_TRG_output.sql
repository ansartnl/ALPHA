...
DROP TRIGGER IF EXISTS airport_id_trg ON airport CASCADE;
CREATE OR REPLACE FUNCTION trigger_fct_airport_id_trg () RETURNS trigger AS $BODY$
BEGIN
  if coalesce(NEW.id::text, '') = '' then
    SELECT nextval('airport_id_seq') INTO NEW.ID ;
  end if;
RETURN NEW;
END
$BODY$
...
CREATE TRIGGER airport_id_trg
	BEFORE INSERT ON airport FOR EACH ROW
	EXECUTE PROCEDURE trigger_fct_airport_id_trg();
