...
DROP TRIGGER IF EXISTS holding_lt ON holding CASCADE;
CREATE OR REPLACE FUNCTION trigger_fct_holding_lt () RETURNS trigger AS $BODY$
BEGIN
    -- Touch the row
      update sys_tables_info set lasttouch = now() AT TIME ZONE 'UTC' where tablename ='HOLDING';
    RETURN NEW;
END
$BODY$
 LANGUAGE 'plpgsql';

CREATE TRIGGER holding_lt
	AFTER INSERT OR UPDATE OR DELETE ON holding FOR EACH ROW
	EXECUTE PROCEDURE trigger_fct_holding_lt();
