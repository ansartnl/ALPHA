...
DROP TRIGGER IF EXISTS rpl_lt ON rpl CASCADE;
CREATE OR REPLACE FUNCTION trigger_fct_rpl_lt () RETURNS trigger AS $BODY$
BEGIN
    -- Touch the row
      update sys_tables_info set lasttouch = now() AT TIME ZONE 'UTC' where tablename ='RPL';
    RETURN NEW;
END
$BODY$
 LANGUAGE 'plpgsql';

CREATE TRIGGER rpl_lt
	AFTER INSERT OR UPDATE OR DELETE ON rpl FOR EACH ROW
	EXECUTE PROCEDURE trigger_fct_rpl_lt();
