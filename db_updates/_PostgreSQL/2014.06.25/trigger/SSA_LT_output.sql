...
DROP TRIGGER IF EXISTS ssa_lt ON ssa CASCADE;
CREATE OR REPLACE FUNCTION trigger_fct_ssa_lt () RETURNS trigger AS $BODY$
BEGIN
    -- Touch the row
      update sys_tables_info set lasttouch = now() AT TIME ZONE 'UTC' where tablename ='SSA';
    RETURN NEW;
END
$BODY$
 LANGUAGE 'plpgsql';

CREATE TRIGGER ssa_lt
	AFTER INSERT OR UPDATE OR DELETE ON ssa FOR EACH ROW
	EXECUTE PROCEDURE trigger_fct_ssa_lt();
