...
DROP TRIGGER IF EXISTS point_lt ON point CASCADE;
CREATE OR REPLACE FUNCTION trigger_fct_point_lt () RETURNS trigger AS $BODY$
BEGIN
    -- Touch the row
      update sys_tables_info set lasttouch = now() AT TIME ZONE 'UTC' where tablename ='POINT';
    RETURN NEW;
END
$BODY$
 LANGUAGE 'plpgsql';

CREATE TRIGGER point_lt
	AFTER INSERT OR UPDATE OR DELETE ON point FOR EACH ROW
	EXECUTE PROCEDURE trigger_fct_point_lt();
