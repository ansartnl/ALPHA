...
QtRedis::QtRedis(QString host, int port, QObject *parent) : QObject(parent)
{
    ...
    reader = new Reader(host, port, this);
...
{
    QProcess process;
    process.start("ping", QStringList() << "-c" << "1" << "-W" << "1" << host);
    process.waitForFinished();
    if(reader && reader->isConnected() && process.exitCode() == 0)
        return true;
    return false;
}
...
void QtRedis::response(QVariant response)
{
    ...
    else if ((res.size() > 1) && (res[1] == "pmessage"))
    {
        reply.type = res[0];
        reply.message = res[1];
        reply.pattern = res[2];
...
QtRedis::Reply QtRedis::command(QString cmd)
{
    ...
    if (cmd != "ROLE")
    {
        qDebug() << "REDIS: " << QString("%1ms").arg(timer.elapsed()) << cmd << responseData;
    }

    Reply reply;

    if((responseData.size() > 0) && (responseData[0] == "integer"))
    {
        reply.type = responseData[0];
        reply.value = responseData[1].toInt();
    }
    else if((responseData.size() > 0) && (responseData[0] == "list"))
    {
        reply.type = responseData[0];
...
bool QtRedis::isValidReply(const Reply &reply) const
{
    bool bRes = true;
    QStringList list = reply.value.toStringList();
    foreach(const QString &s, list)
    {
        if( s.isEmpty() || (reply.value.toString() == "OK") || (reply.value.toString() == "QUEUED") )
            bRes = true;
        else
        {
            bRes = false;
    ...
    if(!bRes)
    {
        qDebug() << "invalidReply" << responseData;
    }
    return bRes;
}
