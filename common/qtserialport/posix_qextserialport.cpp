...
*/
Posix_QextSerialPort::Posix_QextSerialPort(QObject *parent) : QextSerialBase(parent)
{
...
Copy constructor.
*/
Posix_QextSerialPort::Posix_QextSerialPort(const Posix_QextSerialPort& s, QObject *parent) : QextSerialBase(s.port, parent)
{
...
*/
Posix_QextSerialPort::Posix_QextSerialPort(const QString & name, QObject *parent) : QextSerialBase(name, parent)
{
    Posix_File=new QFile();
}
...
*/
Posix_QextSerialPort::Posix_QextSerialPort(const PortSettings& settings, QObject *parent) : QextSerialBase(parent())
{
...
*/
Posix_QextSerialPort::Posix_QextSerialPort(const QString & name, const PortSettings& settings, QObject *parent) : QextSerialBase(name, parent)
{
    setBaudRate(settings.BaudRate);
    setDataBits(settings.DataBits);
    setParity(settings.Parity);
...
\endverbatim
*/
bool Posix_QextSerialPort::setBaudRate(BaudRate baudRate)
{
    LOCK_MUTEX();
    if (Settings.BaudRate!=baudRate) {
        switch (baudRate) {
            ...
            case BaudRate56000:
                Settings.BaudRate=BaudRate38400;
                break;

            case BaudRate76800:

#ifndef B76800
                Settings.BaudRate=BaudRate57600;
#else
            ...
            case BaudRate128000:
            case BaudRate256000:
                Settings.BaudRate=BaudRate115200;
                break;
