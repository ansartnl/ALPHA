...
*/
Win_QextSerialPort::Win_QextSerialPort(QObject *parent) : QextSerialBase(parent)
{
...
Copy constructor.
*/
Win_QextSerialPort::Win_QextSerialPort(const Win_QextSerialPort& s, QObject *parent) : QextSerialBase(s.port, parent)
{
    Win_Handle=INVALID_HANDLE_VALUE;
...
*/
Win_QextSerialPort::Win_QextSerialPort(const QString &name, QObject *parent) : QextSerialBase(name, parent)
{
    Win_Handle=INVALID_HANDLE_VALUE;
}
...
*/
Win_QextSerialPort::Win_QextSerialPort(const PortSettings& settings, QObject *parent) : QextSerialBase(parent)
{
    Win_Handle=INVALID_HANDLE_VALUE;
...
*/
Win_QextSerialPort::Win_QextSerialPort(const QString & name, const PortSettings& settings, QObject *parent) : QextSerialBase(parent)
{
    Win_Handle=INVALID_HANDLE_VALUE;
    setPortName(name);
    setBaudRate(settings.baudRate);
...
qint64 Win_QextSerialPort::size() const
{
    DWORD errorMask = 0;
    COMSTAT comStat = {0};
    if (::ClearCommError(Win_Handle, &errorMask, &comStat)) {
        if (errorMask)
            qWarning() << "Win_QextSerialPort::size() errorMask = " << errorMask;
...
qint64 Win_QextSerialPort::bytesAvailable()
{
    ...
    if (isOpen()) {
        result = size();
        if (-1 != result)
            result += QIODevice::bytesAvailable();
    }
    UNLOCK_MUTEX();
...
\endverbatim
*/
bool Win_QextSerialPort::setBaudRate(BaudRate newBaudRate)
{
    LOCK_MUTEX();
    if (isOpen()) {
        DWORD oldRate = Win_CommConfig.dcb.BaudRate;
        switch (newBaudRate) {
            ...
            default:
                qWarning() << "The " << newBaudRate << " is not supported";
                UNLOCK_MUTEX();
                return false;
        }
        if (!SetCommConfig(Win_Handle, &Win_CommConfig, sizeof(COMMCONFIG)))
        {
            qCritical() << "Unable to set " << newBaudRate << ". API failed with error: " << GetLastError();
            Win_CommConfig.dcb.BaudRate = oldRate;
            UNLOCK_MUTEX();
            return false;
        }
    } else {
        qWarning() << "Unable to set " << newBaudRate << " bauld rate: device is not opened.";
        UNLOCK_MUTEX();
        return false;
    }
