...
QVariantMap SectorRedis::toVariantMap2() const
{
    QVariantMap ret;

    const QStringList clients = this->clients();
    foreach (const QString &client, clients) {
        QVariantMap map;
        const QStringList sectors = getClientSectors(client);
        foreach (const QString &sector, sectors) {
            const SectorR sr = mSectorMap[sector];
            map.insert(sector, sr.GetStatus());
        }
        ret.insert(client, map);
    }

    return ret;
}

void SectorRedis::fromVariantMap2(const QVariantMap &map)
{
    mSectorMap.clear();
    const QStringList clients = map.keys();
    foreach (const QString &client, clients) {
        const QVariantMap m = map[client].toMap();
        const QStringList sectors = m.keys();
        foreach (const QString &sector, sectors) {
            SectorR sr;
            sr.SetClient(client);
            sr.SetStatus(m[sector].toInt());
            mSectorMap.insert(sector, sr);
        }
    }
}
...
QStringList SectorRedis::clients() const
{
    QSet < QString > ret;

    foreach (const SectorR &sr, mSectorMap) {
        ret.insert(sr.GetClient());
    }

    return ret.toList();
}

QStringList SectorRedis::getMasterSectors() const
{
    QStringList sl;
    for(TSectorMap::const_iterator itSector = mSectorMap.constBegin(); itSector != mSectorMap.constEnd(); ++itSector)
    {
