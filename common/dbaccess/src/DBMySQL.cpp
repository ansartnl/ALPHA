...
namespace db
{
    ...
    void CDBMYSQLQuery::BindValue(const QString& placeholder, const QVariant& val, QSql::ParamType type)
    {
        TPlaceMap::iterator itFind = m_PlaceHolders.find(placeholder);
        if (itFind != m_PlaceHolders.end())
            bindValue(itFind.value(), val, type);
    }

    QString CDBMYSQLQuery::GetSystimestampStr() const
    {
        return "now()";
    }
}
