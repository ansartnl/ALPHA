...
void FPL::prepareInsert(QSqlQuery& query)
{
...
void FPL::updateId(QSqlQuery& query)
{
    if (query.driver()->hasFeature(QSqlDriver::LastInsertId)) {
        mId = query.lastInsertId().toInt();
    } else {
        mId = query.boundValue(":"TBL_FPL_ID).toInt();
    }
}

/** Fill query for UPDATE request */
void FPL::prepareUpdate(QSqlQuery& query)
{
...
void FPL::prepareSelect(QSqlQuery& query)
{
...
void FPL::updateInfo(QSqlQuery& query)
{
...
void FPL::updateInfo(const QSqlRecord& record)
{
    ...
    mLit = record.value("lit").toString();
}

/** Fill query for DELETE request */
void FPL::prepareDelete(QSqlQuery& query)
{
...
void FPL::updateQuery(QSqlQuery& query)
{
    //mDepartureTime.setDate(QDateTime::currentDateTime().toUTC().date());

    query.bindValue(":" TBL_FPL_ID, mId, QSql::InOut);
...
void FPL::prepareSelectAll(QSqlQuery& query)
{
    query.prepare("SELECT "
                  TBL_FPL_ID ", "
                  TBL_FPL_FIR_NAME ", "
...
const QUuid& FPL::uid() const
{
    return mUid;
}
void FPL::setUid(const QUuid& uid)
{
    mUid = uid;
}
...
/** Departure Time */
const QTime FPL::ETD() const
{
    return mETD;
}
bool FPL::setETD(const QTime& etd)
{
    mETD = etd;
    return true;
}
/** Arrival Time */
const QTime FPL::ETA() const
{
    return mETA;
}
bool FPL::setETA(const QTime& eta)
{
    mETA = eta;
    return true;
}
...
QVariantMap FPL::toVariantMap() const
{
    QVariantMap map;
    map.insert("id", QString::number(mId));
    map.insert("Uid", mUid.toString());
    map.insert("FIRname", mFIRName);
    map.insert("ACID", mAircraftId);
...
void FPL::fromVariantMap(const QVariantMap& map)
{
    mId = map.value("id", mId).toInt();
    mUid = QUuid(map.value("Uid", mUid.toString()).toString());
    mFIRName = map.value("FIRname", mFIRName).toString();
    mAircraftId = map.value("ACID", mAircraftId).toString();

    // ...
}
bool FPL::setLit(const QString& sValue)
{
    QString sLit = sValue.toUpper();

    mLit = sLit;
    return true;
}
const QString& FPL::lit() const
{
    return mLit;
}
